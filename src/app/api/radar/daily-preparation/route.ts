...
import { latestPreparation, startPreparation, retryPreparation } from "@/app/modules/stories/daily-preparation.repository";
  ...
  try {
    ...
    if(write) {
      ...
      if(input.action==="retry") {
        ...
        run=(await startPreparation(topicId,line.id,line.name,timezone,input.mode ?? "day")).run;
      } else return noStoreJson({error:"Choose start or retry"},400);
    }
