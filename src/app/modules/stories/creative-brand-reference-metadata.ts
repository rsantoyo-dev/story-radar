 ...
 */
export function parseCreativeBrandReferencePatch(input: unknown): Partial<
  CreativeBrandReferenceMetadata & { isActive: boolean }
> {
  const raw = asRecord(input) as RawMetadata & { isActive?: unknown };
  const patch: Partial<
    CreativeBrandReferenceMetadata & { isActive: boolean }
  > = {};
