...
/**
 * Upper bound on the facts a brief may carry. A bound, not a target: it only
...
- every previousFeedback entry with severity "blocker" must be fully resolved in the returned draft; do not return accepted or revised while any blocker remains. Apply these fixes:
  ...
  - BODY_TOO_LONG: cut the supporting text to 40 words or fewer (the hard limit is 45, and counts drift upward) without dropping a qualifier, attribution, or scope phrase
...
async function generateUnreviewedCreativeBrief({
  ...
  const requestBrief = async (
    ...
    const result = await generateJson({
      ...
      paidGeminiApiKey,
      openAiApiKey,
      openAiModel: openAiEditorialModels?.minorRepairModel ?? "gpt-6-sol",
      openAiAuditContext,
      model,
      primaryProvider,
      groqApiKey,
      ...
      // Sized for MAX_BRIEF_KEY_FACTS facts with claim guards; a cap, so an
      // ordinary brief costs no more than before.
...
export async function generateCreativeDraft(options: GenerateDraftOptions): Promise<GeneratedCreativeDraftResult> {
  ...
  const repaired = await repairRemainingCreativeBlockers(enforceCoverTitle(generated.draft, options.profile.requireCoverTitle, options.brief.contentTitle ?? options.story.title), {
    ...
    openAiModel: options.openAiEditorialModels?.minorRepairModel ?? "gpt-6-sol",
...
async function generateReviewedCreativeDraft({
  ...
  let response = await generateJson({
    ...
    paidGeminiApiKey,
    openAiApiKey,
    openAiModel: writerModel ?? openAiEditorialModels?.minorRepairModel ?? "gpt-6-sol",
...
export function strictCreativeSchema(schema: Record<string, unknown>): Record<string, unknown> {
  const result = { ...schema };
...
function parseCreativeBrief(
  ...
  // Same bound as the schema: a response the provider was allowed to produce
  // must not be rejected here (a hardcoded 6 did exactly that once the
  // schema ceiling was raised).
  const keyFacts = arrayValue(value.keyFacts, "keyFacts", 1, MAX_BRIEF_KEY_FACTS).map(
    (item, index) => {
