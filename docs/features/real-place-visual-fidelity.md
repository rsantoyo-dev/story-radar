...
- `CREATIVE_GEO_OVERPASS_URL` permite configurar otra instancia HTTPS pública o propia accesible con DNS público. Vacío usa `https://overpass-api.de/api/interpreter`. Límite conservador: 8 consultas por día UTC **por proceso**, 2 MB por respuesta (la consulta pide `out tags geom(bbox)`: tags y geometría recortada al encuadre, sin listas de nodos ni bounds), 18 segundos por solicitud, caché de 30 minutos y 16 entradas. No es una cuota distribuida ni un servicio con SLA: antes de escalar, provisionar capacidad y coordinación de cuotas.
- El renderizador admite localizaciones puntuales y segmentos verificados en ambos hemisferios. Limita la extensión local a 12 km proyectados para puntos y 18 km para tramos (el encuadre de un tramo descuenta las bandas de título, leyenda y atribución para que sus dos extremos queden visibles); por encima de 8 km de ancho consulta solo vías principales (motorway/trunk/primary/secondary), porque a esa escala un lienzo de 944 px no puede mostrar calles residenciales legibles y la respuesta se mantiene dentro de los mismos topes. Excluye polos/antimeridiano y rechaza respuestas incompletas. Es un mapa de contexto con calles, no un mapa de navegación ni una prueba del estado actual.
- `CREATIVE_GEO_SOURCE_ADAPTERS="quebec511"` habilita el adaptador opcional de la fuente 511/MTMD; vacío lo deshabilita. Cubre las fichas 511 y los comunicados del propio ministerio (`quebec.ca`, `transports.gouv.qc.ca`); la geometría siempre procede del WFS oficial, nunca de la página. Una ficha 511 exige coincidencia exacta del aviso, tramo, dirección y fechas; un comunicado en prosa se resuelve contra un único chantier (ver «Comunicados MTMD» más abajo). Un fallo del adaptador no convierte el tramo en un marcador del centro municipal. Otros países usan el recorrido general; nuevas fuentes oficiales pueden aportar adaptadores propios.
...
## Comunicados MTMD y chantiers oficiales — 23 de septiembre de 2026

Caso: «Fermetures du boulevard Marie-Victorin, en direction sud, à Brossard, dans les nuits du 28 septembre au 7 octobre 2026», un comunicado CNW del ministerio publicado en `quebec.ca/nouvelles`. Hasta ahora el adaptador solo aceptaba `511.gouv.qc.ca` y su coincidencia exigía el registro tabular de una ficha 511, así que un comunicado nunca llegaba al WFS y la slide «carte» acababa en tipografía o en ilustración.

Ahora `quebec-road-map.ts` distingue dos tipos de evidencia:

- **Ficha 511** (`hasRoadNoticeRecord`): recorrido anterior, sin cambios — misma ruta, localización, dirección y fechas exactas en un único registro citado por la slide.
- **Comunicado en prosa** (`matchOfficialChantier`): lee de los hechos del brief pares ruta+dirección en la misma cláusula (para que el «direction est» de un desvío no se pegue a la ruta cerrada), la ventana de fechas con año explícito, el municipio y al menos un punto nombrado de la `localisation` oficial, y el tipo de cierre (completo o de vías). Los registros que superan todo se agrupan por `identifiantChantier`; se exige **exactamente un** chantier, con geometría idéntica en sus filas nocturnas. Sin año, sin ruta con dirección, con dos chantiers o con geometrías distintas no hay mapa, y el motivo lo dice.

El feed publica las entraves **noche por noche**. Un comunicado emitido días antes del cierre encuentra el chantier (`319446` en este caso) pero ninguna fila que cubra su ventana; el motivo guardado es «Official work site 319446 matches… but its published windows do not cover 2026-09-28 to 2026-10-07 yet», y la preparación puede repetirse cerca de la fecha. La comprobación al aprobar/exportar (`roadMapStillCurrent`) compara chantier, ruta, dirección, localización, redacción, desvío y geometría — no el id de fila ni sus horas — para que la rotación nocturna no marque como caducado un mapa correcto, y sí lo haga un cambio de desvío o de trazado.

La leyenda del mapa repite la redacción oficial (`entrave` y `detoursEtItinerairesFacultatifs`, p. ej. «Route fermée · Détour : Sortie 75, boulevard Marie-Victorin»). El desvío se nombra, nunca se dibuja: GMAP-04 prohíbe sustituir geometría acreditada por rutas calculadas, y el WFS no publica el trazado del desvío. Fixture: `fixtures/quebec-road-brossard-15.json`, captura real del feed del 23 de septiembre con los chantiers de Brossard y avisos homónimos de Boucherville y Sorel-Tracy como distractores.

`visualNeed: "verified-map"` del guion ya enruta la slide al recorrido documental (`requiresVerifiedGeography`), aunque su `visualDirection` no diga «carte». Sin geometría verificable, la slide cae en ilustración conceptual como antes; nunca en un mapa dibujado.

Primera prueba real (23 de septiembre, historia de prueba sembrada con el cierre nocturno de la A-30 en Brossard, fila 172662): el brief resolvía el chantier 322624, pero no salió mapa por dos huecos. `creative_units` no tiene columna `visual_need`, así que el valor del writer solo sobrevive en `ai_snapshot` y `mapCreativeDraft` no lo leía; ahora `creative-visual-need.ts` lo recupera por orden de unidad, la misma alineación que usa `generatedDraftCopyMatches`. Y el writer devolvía `generic-illustration` en todas las slides porque la instrucción solo decía cuándo *no* pedir mapa: la versión `carousel-draft-v50` le pide declarar `verified-map` en la slide que localiza un tramo, desvío, dirección o sede nombrados, escribiendo su `visualDirection` como fallback abstracto. La evidencia del adaptador (aviso, chantier, localización, desvío) se muestra ahora bajo la imagen en el bloque *Place material*.

Comprobación sin proveedores de pago: `npx tsx --env-file=.env.local scripts/road-map-check.mts <storyId> [--render out.png]` reproduce la decisión del adaptador para una historia (señales leídas, motivo, chantier) con el feed vivo; `--row <identifiant> --render out.png` dibuja cualquier fila activa para revisar el renderizador. Verificado el 23 de septiembre: Marie-Victorin devuelve el motivo «no cubre todavía» y el comunicado de la A-13/A-520 (también del 28) aún no tiene filas.

### `visualNeed: "real-photo"` conectado al enrutador — 24 de septiembre de 2026

Caso: «Le pont Gouin sera complètement fermé le 28 septembre pour une inspection» (`sjsr.ca`, ville de Saint-Jean-sur-Richelieu, fuera del catálogo MTMD/511). El writer escribió correctamente una dirección de ilustración plana idéntica en las 4 slides, evitando a propósito dibujar el puente por falta de evidencia verificada — pero `requiresVerifiedGeography()` solo reconocía `visualNeed: "verified-map"`; `"real-photo"` existía en el tipo desde antes pero no enrutaba a nada, así que `composeDraftPlaceVisuals`/`preparePlaceVisuals` nunca se llamaba y ninguna slide intentaba resolución de lugar.

Verificado en vivo (Wikidata, gratuito, sin clave): existen dos entidades «Pont Gouin», una en Irlande (Québec, municipio distinto) y `Q16510463`, ubicada en `Q141977` = Saint-Jean-sur-Richelieu, con coordenadas correctas y dos fotos CC en Commons — una de ellas, «Fermeture du pont Gouin.JPG», es literalmente una foto de un cierre *anterior* (inundación de 2011), CC0, con atribución completa. Esto confirma que el puente es resoluble por identidad; también confirma por qué esa foto en particular **no debe** usarse como prueba del cierre de 2026: es de otro evento, catorce años antes.

`requiresVerifiedGeography()` ahora también acepta `"real-photo"`. La instrucción del writer (`carousel-draft-v51`) le pide declarar `real-photo` en la slide que nombra un lugar real reconocible (puente, edificio, monumento), incluso sin saber si hay evidencia disponible — la resolución la hace el pipeline, nunca el writer. El descubrimiento de `purpose` sigue decidiendo foto vs. mapa como ya hacía: `current-state` (cualquier hecho con «fermé», «travaux», etc.) excluye la foto de archivo como prueba y cae a un mapa de ubicación; solo `purpose: "location"` permite una foto de archivo como contexto de identidad. Declarar `real-photo` nunca inventa evidencia: sin resolución verificable, la slide usa el mismo *fallback* conceptual de siempre.

### Foto de archivo como referencia de identidad en un cierre — 24 de septiembre de 2026

Continuación del caso `real-photo` de pont Gouin. Probado en vivo contra la coordenada real de Wikidata (`Q16510463`): su precisión es `0.0128°` ≈ 1,4 km (128 veces peor que el umbral `0.0001°` ≈ 11 m que exige `validCoordinate`) — Wikidata nunca guardó una coordenada fina para este puente, así que ni el mapa ni la foto-como-evidencia podían resolver, aunque la identidad sí resolvía perfecto.

Decisión de producto: cuando el writer declaró `real-photo` para una slide sobre un cierre (`purpose: "current-state"`), y existe una foto de archivo elegible (licencia CC verificada) para el lugar identificado, esa foto puede fundamentar una adaptación de IA honesta de la identidad del lugar — nunca como prueba de que el cierre está pasando. Es una vía distinta, y paralela, del uso ya existente de `purpose: "location"` (donde la foto es evidencia directa): la regla *"para cambios de estado se excluye la foto como prueba"* no cambia; lo que cambia es que ahora hay una segunda vía — *foto como referencia de identidad para una ilustración* — que sí puede correr aunque sea `current-state`.

Implementación: `prepare-place-visuals.ts` intenta la foto (antes del mapa) cuando `extraction.purpose !== "location" && unit.visualNeed === "real-photo"`; si es elegible, marca `generationUse: "ai-reference"` y una razón explícita ("used only to ground the place's identity... not evidence of current conditions"). `manage-creative-assets.ts` reconoce esa marca incondicionalmente (sin el flag experimental `photoReferenceTest`, que sigue existiendo solo para el camino `location` histórico), gobernada por el mismo dial `CREATIVE_MAP_REFERENCE_MODE` que ya decide mapas como referencia de IA vs. composición local — un solo interruptor de operador para todo material geográfico verificado compuesto como referencia. El texto que recibe el modelo prohíbe explícitamente inventar *"event attendance, damage, closures, barriers, detour signage"* sobre la foto real — el riesgo específico de usar una foto antigua para una historia de cierre es que el modelo le añada una barrera o señal de cierre que la foto original no tiene. Los bytes se guardan en R2 (`storeDocumentaryPhotoReference`, mismo patrón que el mapa) antes de enviarse a fal, para que el generador y un revisor puedan leerlos de vuelta.

La subida manual de fotos por parte de un editor (`StoryPhotosPanel`, propósito `place`) sigue siendo la vía cuando la búsqueda automática (Wikidata/Commons, Google) no encuentra nada — ninguna de las dos rutas altera una foto sin permiso claro de uso; la casilla de permiso en la subida manual y la licencia CC verificada en la automática cubren esa misma exigencia por caminos distintos.

Aviso proactivo: cuando `placeVisual.representation === "typography"` está presente (la slide necesitaba evidencia verificada y no la obtuvo, cualquiera sea el motivo — identidad ambigua, sin foto/mapa elegible, o un proveedor externo caído), el workspace muestra bajo la imagen: *"I did not find a real or recent image of [lugar]. If you have one, please upload it and recompose this image — otherwise a generic illustration is used, as shown here."*, con un enlace directo al selector de fotos de esa slide (ancla `#unit-photos-<order>`). Nombra el lugar cuando la identidad sí resolvió (aunque nada más lo hiciera); si ni la identidad resolvió (p. ej. Wikidata con `maxlag` excedido), omite el nombre.

Caso real que motivó esto (24 de septiembre): con la identidad de pont Gouin ya resuelta pero sin coordenada precisa ni foto de archivo elegible en un intento, un segundo intento chocó con el propio Wikidata reportando `"maxlag": 403 segundos` de retraso de replicación — el código pide `maxlag=5` (cortesía estándar hacia la infraestructura pública) y Wikidata rechaza la solicitud mientras dure el retraso. No es un fallo del código; es el estado transitorio de un servicio externo compartido, y es exactamente el caso donde la subida manual (o un reintento más tarde) es la única vía.

### Slide de mapa compuesta por la IA — 24 de septiembre de 2026

Decisión de producto: la slide con mapa verificado debe llevar el mismo diseño editorial que el resto del carrusel, no la tarjeta tipográfica local. Por defecto (`CREATIVE_MAP_REFERENCE_MODE` distinto de `local`), el PNG exacto que renderiza el adaptador se guarda como original privado (`originals/<sha256>`, igual que las fotos de archivo), se entrega al modelo de imagen como **última** imagen de referencia por el endpoint `reference-guided`, y el prompt le exige reproducir el panel tal cual — mismas calles, etiquetas, leyenda y tramo rojo, sin añadir nada — y construir el diseño alrededor. La evidencia queda como `generationUse: "ai-reference"` con `adapterEvidence` intacta (aviso, chantier, re-verificación al aprobar/exportar) y un motivo que pide **comparar el panel con el original antes de aprobar**: un modelo de imagen redibuja lo que recibe, y la fidelidad la certifica la revisión humana, no el prompt. Con `CREATIVE_MAP_REFERENCE_MODE=local`, o en topics `photo-required` / `verified-references`, se conserva la composición determinista con los píxeles exactos. La recomposición individual (*Recompose*) sigue usando la composición local.

Pendiente: geometrías MultiLineString, comunicados que citen varias rutas cerradas bajo chantiers distintos (hoy: ambiguo, sin mapa), un reintento programado de la preparación cuando el feed publique la ventana anunciada, y mostrar el original guardado junto a la slide generada para la comparación.
