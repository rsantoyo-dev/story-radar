# Tratamientos visuales editoriales para la grilla — FEAT-VISUAL-001

## Backlog

## Por hacer

### VISUAL-01 — Definir tratamientos y compatibilidad

  - priority: high
  - tags: [creative, visual, taxonomy, migration, p0]
  - note: catálogo y cuotas son configurables por creative profile/topic; character-explainer no presupone Richard

### VISUAL-02 — Seleccionar tratamiento según historia y cartera

  - priority: high
  - tags: [creative, visual, editorial, p0]
  - depends-on: [VISUAL-01, ANGLE-01]
  - note: usa el ángulo autoritativo del brief; la cuota 40/25/15/10/10 es solo el seed de Tech

### VISUAL-03 — Actualizar las instrucciones de generación

  - priority: high
  - tags: [creative, visual, prompts, p0]
  - depends-on: [VISUAL-01, VISUAL-02]

### VISUAL-04 — Permitir revisión y override humano

  - priority: medium
  - tags: [creative, visual, ui]
  - depends-on: [VISUAL-02, VISUAL-03]

### VISUAL-05 — Versionar snapshot y conservar históricos

  - priority: high
  - tags: [creative, visual, versions, p0]
  - depends-on: [VISUAL-03]

### VISUAL-06 — Verificación automatizada y revisión visual

  - priority: high
  - tags: [creative, visual, tests, p0]
  - depends-on: [VISUAL-01, VISUAL-02, VISUAL-03, VISUAL-05]

## En progreso

## En revisión / QA

## Hecho
