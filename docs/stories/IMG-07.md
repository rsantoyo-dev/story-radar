...
- Pendiente concreto: si el envío al proveedor tiene éxito pero la generación **falla más tarde** (el poller la marca `failed`), la solicitud queda como `applied` con una imagen rota en la unidad; el poller debe propagar ese fallo a `creative_asset_edit_requests`. El fallo **síncrono** al enviar ya se maneja (marca `failed`, descarta la versión, permite "Reintentar").
